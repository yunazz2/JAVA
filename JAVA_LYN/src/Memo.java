...
/** [개념]
 ...
 - 생성자(constructor) : new 연산자로 호출되는 특별한 중괄호 블록. 객체 생성 시 객체를 초과하는 역할을 한다. 메소드와 비슷하게 생겼지만, 클래스 이름으로 되어 있고 반환형이 없다. 객체 초기화란 필드를 초기화하거나, 메소드를 호출해서 객체를 사용할 준비를 하
	는 것을 말한다.
 - 오버 라이딩 : 상위 클래스의 메서드를 하위 클래스가 재정의 하는 것이다.
 ...
 - toString : 객체를 출력할 때, 지정한 문자열 형식으로 반환하는 메소드로 object(최상위 클래스)에 정의되어 있으며, 이를 오버라이딩 하여 사용한다.
 - 템플릿 : 견본, 틀
	- 템플릿 메소드 : 일련의 작업 수행을 하는 메소드들을 특정 순서로 호출하는 메소드로, 전체적인 수행 구조를 변경하지는 않으면서, 특정 단계의 구현 기능을 변경하는 패턴
     => abstract 추상 메소드를 응용하여 정의하고, 구현 클래스에서 다형성을 실현하여 적용한다.
 - 인터페이스 : 추상 메소드만 사용할 수 있으나(= 중괄호로 된 메소드 사용 못함), default와 static 메소드는 사용할 수 있도록 허용한다.
	- 인터페이스 내에서의 디폴트 메소드 : 구현한 객체가 기본으로 사용할 수 있는 메소드(보이지는 않지만 이미 오버라이딩 되어있다.)
 - 변수 유형에 따른 용도 : 
    - 지역 변수 : 메소드, 블록{} 내부에 선언
				{} 블록 내부에서만 사용 / 스택 영역
 	- 멤버 변수 : 클래스의 변수 선언
...
/* [Memo]
 ...
 - static이면 class 이름으로 바로 직접 접근 가능, 아니면 객체를 new로 생성해서 접근해야 함.
 - 객체를 생성하려면 클래스 타입을 알아야 함.
 */
